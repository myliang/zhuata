    ...
    class Xseight
      class << self
        def parse(doc, record)
          ...
          record.new_tags = doc.css('.bookinfo .info1 a')[0].text
          record.text = doc.css('#BookIntro')[0].text

          chapter_url = "http://www.xs8.cn#{doc.css('.readbtn a')[0]['href']}"
        ...
        def read(doc)
          doc.css('.chapter_content')[0].inner_html
        end
      end
    end
  end
